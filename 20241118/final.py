...
# 創建新的 USD Stage
stage = Usd.Stage.CreateNew("20241118/lab.usd")
...
chair_offset_left = (-50, 0, 0)  # 椅子放置在桌子左側的偏移量
chair_offset_right = (50, 0, 0)  # 椅子放置在桌子右側的偏移量
...
for row in range(5):  # 共有 5 排
    for column in range(4):  # 每排 4 張桌子
        ...
        # 在每排添加椅子
        if row in [0, 2]:  # 第 1、3 排，椅子放左側
            chair_name = f"Chair_Row{row + 1}_Col{column + 1}"  # e.g., Chair_Row1_Col1
            chair_path = f"/World/{chair_name}"
            chair_prim = stage.DefinePrim(chair_path, "Xform")  # 創建椅子容器 Prim
            references = chair_prim.GetReferences()  # 獲取引用接口
            references.AddReference('../asset/chair.usdz')  # 引用外部椅子資產

            # 設定椅子的位置（桌子左側）
            chair_xform = UsdGeom.Xformable(chair_prim)
            chair_xform.AddTranslateOp().Set((
                position[0] + chair_offset_left[0],
                position[1] + chair_offset_left[1],
                position[2] + chair_offset_left[2]
            ))
            chair_xform.AddScaleOp().Set((1, 1, 1))  # 保持原大小
            chair_xform.AddRotateXYZOp().Set(Gf.Vec3d(90, 0, 90))  # 反方向

        elif row in [1, 3]:  # 第 2、4 排，椅子放右側
            chair_name = f"Chair_Row{row + 1}_Col{column + 1}"  # e.g., Chair_Row2_Col1
            chair_path = f"/World/{chair_name}"
            chair_prim = stage.DefinePrim(chair_path, "Xform")  # 創建椅子容器 Prim
            references = chair_prim.GetReferences()  # 獲取引用接口
            references.AddReference('../asset/chair.usdz')  # 引用外部椅子資產
            ...
            chair_xform = UsdGeom.Xformable(chair_prim)
            chair_xform.AddTranslateOp().Set((
                position[0] + chair_offset_right[0],
                position[1] + chair_offset_right[1],
                position[2] + chair_offset_right[2]
            ))
            chair_xform.AddScaleOp().Set((1, 1, 1))  # 保持原大小
            chair_xform.AddRotateXYZOp().Set(Gf.Vec3d(90, 0, -90))  # 反方向

        elif row == 4:  # 第 5 排，椅子放右側（模仿第二排）
            chair_name = f"Chair_Row{row + 1}_Col{column + 1}"  # e.g., Chair_Row5_Col1
            chair_path = f"/World/{chair_name}"
            chair_prim = stage.DefinePrim(chair_path, "Xform")  # 創建椅子容器 Prim
            references = chair_prim.GetReferences()  # 獲取引用接口
            references.AddReference('../asset/chair.usdz')  # 引用外部椅子資產
...
print("完整的 USD 場景已成功創建：office_layout_with_chairs.usd")
